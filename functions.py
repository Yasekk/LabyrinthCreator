...
def mouse_click(walls_h, walls_v, grid_walls_h, grid_walls_v, settings,
	...
	if save_button.rect.collidepoint(mouse_x, mouse_y):
		...
		with open("walls_v.json", "w") as wall_v_save:
			#Czyszczenie pliku "walls_v" i tworzenie pustej listy, 
			#w której przechowane będą dane do zapisu.
			wall_v_data = []
		for wall in walls_v:
			#Pobieranie danych położenia z istniejących ścian pionowych 
			#i dodanie tych danych do listy.
			data = wall.save()
		...
		with open("walls_h.json", "w") as wall_h_save:
			#czyszczenie pliku "walls_h" i tworzenie pustej listy,
			#w której przechowane będą dane do zapisu.
			wall_h_data = []
		for wall in walls_h:
			#Pobieranie danych położenia z istniejących ścian poziomych
			#i dodanie tych danych do listy.
			data = wall.save()
		...
		#Jeżeli zostanie klikniety przycisk "load", nastąpi odczytanie 
		#zawartości plików z położeniem ścian.
		with open("walls_v.json", "r") as walls_v_load:
			#Usunięcie wszytkich istniejących ścian pionowych i 
			#wczytanie zapisanego położenia ścian z pliku.
			walls_v.empty()
			walls_v_positions = json.load(walls_v_load)
			for wall_v_position in walls_v_positions:
				#Tworzenie nowych ścian pionowych i przypisanie im 
				#położenia zgodnego z tym, który był zapisany w pliku.
				wall_vertical = WallVertical(settings, screen)
				wall_vertical.rect.centerx = wall_v_position[0]
				wall_vertical.rect.centery = wall_v_position[1]
				walls_v.add(wall_vertical)
		with open("walls_h.json", "r") as walls_h_load:
			...
			for wall_h_position in wall_h_positions:
				...
				#położenia zgodnego z tym, który był zapisany w pliku.
				wall_horizontal = WallHorizontal(settings, screen)
				wall_horizontal.rect.centerx = wall_h_position[0]
				wall_horizontal.rect.centery = wall_h_position[1]
				walls_h.add(wall_horizontal)					
	#Jeżeli miejsce kliknięcia pokryje się ze ścianą, zostanie ona
	#usunięta.
	skip_create = click_delete_wall(walls_h, walls_v, mouse_x, mouse_y)
...
def click_create_wall(grid_walls_h, grid_walls_v, walls_h, walls_v,
                      mouse_x, mouse_y, settings, screen):
